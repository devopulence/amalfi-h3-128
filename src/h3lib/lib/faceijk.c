 ...
 *
 * H3-EXTENDED: extended from length 17 to 23 with element type widened
 * `int → int64_t` per POC-5. Class II pattern is `maxDimByCIIres[2n] =
 * 2 * 7^n`; Class III entries are -1. Values 0-16 are stock H3
 * byte-identical (decoded as int64_t but bit values unchanged for
 * indices that fit int32). Indices 17-22 are computed:
 *   [17] = -1            (Class III)
 *   [18] = 2 * 7^9       = 80,707,214
 *   [19] = -1            (Class III)
 *   [20] = 2 * 7^10      = 564,950,498   (last fitting int32)
 *   [21] = -1            (Class III)
 *   [22] = 2 * 7^11      = 3,954,653,486 (REQUIRES int64 — POC-5 boundary)
 *
 * Element type widened from int → int64_t to hold index 22. Callers
 * (`_adjustOverageClassII`, `_adjustPentVertOverage`) must use
 * int64_t locals when reading from this table; widened in lock-step.
 */
static const int64_t maxDimByCIIres[] = {
    2,            // res  0
    -1,           // res  1
    14,           // res  2
    -1,           // res  3
    ...
    -1,           // res 21 (H3-EXTENDED)
    3954653486LL  // res 22 (H3-EXTENDED, 2 * 7^11) — exceeds INT32_MAX
};

/** @brief unit scale distance table
 *
 * H3-EXTENDED: extended from length 17 to 23 (mirrors maxDimByCIIres),
 * element type widened `int → int64_t`. Class II pattern:
 * `unitScaleByCIIres[2n] = 7^n`; Class III entries -1.
 *   [22] = 7^11         = 1,977,326,743 (single value fits int32 but
 *                        consumers do `* 3` or larger → overflow)
 */
static const int64_t unitScaleByCIIres[] = {
    1,            // res  0
    -1,           // res  1
    7,            // res  2
    -1,           // res  3
    ...
    -1,           // res 21 (H3-EXTENDED)
    1977326743LL  // res 22 (H3-EXTENDED, 7^11)
};
...
void _faceIjkPentToCellBoundary(const FaceIJK *h, int res, int start,
    ...
    for (int vert = start; vert < start + length + additionalIteration;
        ...
        if (isResolutionClassIII(res) && vert > start) {
            ...
            int64_t maxDim = maxDimByCIIres[adjRes];
            Vec2d v0 = {3.0 * (double)maxDim, 0.0};
            Vec2d v1 = {-1.5 * (double)maxDim,
                        3.0 * M_SQRT3_2 * (double)maxDim};
            Vec2d v2 = {-1.5 * (double)maxDim,
                        -3.0 * M_SQRT3_2 * (double)maxDim};
...
Overage _adjustOverageClassII(FaceIJK *fijk, int res, int pentLeading4,
    ...
    int64_t maxDim = maxDimByCIIres[res];
    ...
    else if (ijk->i + ijk->j + ijk->k > maxDim)  // overage
    {
        ...
        CoordIJK transVec = fijkOrient->translate;
        int64_t unitScale = unitScaleByCIIres[res];
